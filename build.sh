...
docker build --platform linux/amd64 -t cross-compiler -f "$(pwd)/Dockerfile" "$(pwd)"

# Run the compilation inside the container
docker run --platform linux/amd64 --rm --entrypoint "" -v "$(pwd)":/work cross-compiler /bin/bash -c "
...
if [ -f "build/TheNeuralGrid_arm" ] && [ -f "build/TheNeuralGrid_arm64" ]; then
    echo "Cross-compilation successful!"
    echo "Binaries created in the 'build' folder: build/TheNeuralGrid_arm, build/TheNeuralGrid_arm64"
else
    echo "Cross-compilation failed! Check logs for details."
fi
